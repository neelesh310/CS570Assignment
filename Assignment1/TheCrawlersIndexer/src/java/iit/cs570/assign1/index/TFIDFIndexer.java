...
public void close() throws CorruptIndexException, IOException
 ...
 private Document getDocument(File file) throws IOException
    ...
    org.jsoup.nodes.Document htmlDoc = Jsoup.parse(file, "utf-8");
     ...
     Field text = new Field(TheCrawlersConstants.TEXT, htmlDoc.text(), Field.Store.YES, Field.Index.NOT_ANALYZED); 
...
@Override
   public int createIndex(String dataDirPath, FileFilter filter) 
      ...
      for (File file : files) {if(!file.isDirectory()&& !file.isHidden()&& file.exists()&& file.canRead()&& filter.accept(file))
        {
            indexFile(file);
         }
      }
